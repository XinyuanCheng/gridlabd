...
STATUS GldLoader::compile_code(CLASS *oclass, int64 functions)
{
	...
	{
		if ( getenv("GRIDLABD") != NULL )
		{
			global_include = getenv("GRIDLABD");
			IN_MYCONTEXT output_verbose("global_include is not set, assuming value of GRIDLABD variable '%s'", (const char*)global_include);
		}
		else
		{
...
int GldLoader::process_macro(char *line, int size, char *_filename, int linenum)
{
	...
	else if ( strncmp(line,"#command",8) == 0 )
	{
		...
		sprintf(command_line,"%s/gridlabd-%s",(const char*)global_execdir,command);
	...
	else if (strncmp(line,"#gridlabd",9)==0)
	{
		...
		strcpy(value, strip_right_white(term+1));
		IN_MYCONTEXT output_debug("%s(%d): executing system(char *cmd='%s %s')", filename, linenum, (const char*)global_execname, value);
		global_return_code = my_instance->subcommand("%s %s",(const char*)global_execname,value);
		if( global_return_code != 0 )
		{
			syntax_error(filename,linenum,"#gridlabd %s -- system('%s %s') failed with status %d", value, (const char*)global_execname, value, global_return_code);
			strcpy(line,"\n");
			return FALSE;
		}
		else
	...
	else if ( strncmp(line, "#version", 8) == 0 )
	{
		...
		while ( (next=strtok_r(next?NULL:line+9," \t",&last)) )
		{
			...
			{
				sprintf(value1,"%s",(const char*)global_version_branch);
	...
	int rc = my_instance->subcommand("%s/" PACKAGE "-%s",(const char*)global_execdir,strchr(line,'#')+1);
	if ( rc != 127 )
	{
		strcpy(line,"\n");
		return rc==0;
